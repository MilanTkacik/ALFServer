...
inline void convertStringToSwtHex(const std::string& string, uint32_t num[3])
{
    if (string.length() > 24)
    {
    ...
    std::stringstream ss;
    ss << std::setw(24) << std::setfill('0') << string;

    num[2] = std::stoul(ss.str().substr(0, 8), NULL, 16);
    num[1] = std::stoul(ss.str().substr(8, 8), NULL, 16);
    num[0] = std::stoul(ss.str().substr(16, 8), NULL, 16);
}

/// Converts a 96-bit hex number to string
inline std::string convertSwtHexToString(const uint32_t num[3])
{
    std::stringstream h, m, l;
    h << std::setw(8) << std::setfill('0') << std::hex << num[2];
    m << std::setw(8) << std::setfill('0') << std::hex << num[1];
    l << std::setw(8) << std::setfill('0') << std::hex << num[0];
