...
struct ServiceDescription
{
    ...
    struct SwtSequence
    {
        std::vector<Swt::SwtData> swtData;
    };
...
class ProgramAliceLowlevelFrontendServer: public AliceO2::Common::Program
{
  public:
    ...
    virtual std::vector<CardDescriptor> getCardsFromFile()
    {
      std::ifstream iFile(getenv(std::string("CRU_LIST_PATH").c_str()));

      if (!iFile.is_open())
      {
          BOOST_THROW_EXCEPTION(ScaException() << ErrorInfo::Message("Cannot open CRU LIST file"));
      }
      ...
      std::string card;
      while (std::getline(iFile, card))
      {
        if (card.length() == 7)
        {
          cards.push_back(CardDescriptor{CardType::Cru, serial, {"cru", "cru"}, PciAddress(card)});
        }
      }
    ...
    virtual void run(const b::program_options::variables_map&) override
    {
      ...
      if (getenv(std::string("CRU_TABLE_PATH").c_str()) == nullptr) {
        BOOST_THROW_EXCEPTION(AlfException() << ErrorInfo::Message("Environment variable 'CRU_TABLE_PATH' not set"));
      }

      if (getenv(std::string("CRU_LIST_PATH").c_str()) == nullptr) {
        BOOST_THROW_EXCEPTION(AlfException() << ErrorInfo::Message("Environment variable 'CRU_LIST_PATH' not set"));
      }

      DimServer::start("ALF");

      std::vector<CardDescriptor> cardsFound = getCardsFromFile();
      ...
      for (auto card : cardsFound) {
        ...
        BarSharedPtr bar0, bar2;
	
	if (card.cardType != CardType::Dummy)
	{
		bar0 = ChannelFactory().getBar(card.pciAddress, 0);
        	bar2 = ChannelFactory().getBar(card.pciAddress, 2);
	}
	else
	{
		bar0 = ChannelFactory().getBar(Parameters::makeParameters(serial, 0));
		bar2 = ChannelFactory().getBar(Parameters::makeParameters(serial, 2));
	}
        ...
        for (auto link : links) {
          ...
          /*servers.push_back(makeServer(names.swtWrite(),
            [bar2, linkInfo](auto parameter){
              return swtWrite(parameter, bar2, linkInfo);}));*/
          /*servers.push_back(makeServer(names.swtRead(),
            [bar2, linkInfo](auto parameter){
              return swtRead(parameter, bar2, linkInfo);}));*/

          servers.push_back(makeServer(names.scaSequence(),
            [bar2, linkInfo](auto parameter){
              return scaBlobWrite(parameter, bar2, linkInfo, true);}));

          servers.push_back(makeServer(names.swtSequence(),
            [bar2, linkInfo](auto parameter){
              return scaBlobWrite(parameter, bar2, linkInfo, false);}));
          ...
          // Publish SWT sequence RPCs
          servers.push_back(makeServer(names.publishSwtSequenceStart(),
            [commandQueue, linkInfo](auto parameter){
              return publishSwtSequenceStart(parameter, commandQueue, linkInfo);}));
          servers.push_back(makeServer(names.publishSwtSequenceStop(),
            [commandQueue, linkInfo](auto parameter){
              return publishSwtSequenceStop(parameter, commandQueue, linkInfo);}));
        }
      }
  ...
  private:
      ...
      Visitor::apply(service->description.type,
        ...
        [&](const ServiceDescription::ScaSequence& type){
          ...
          [&](const ServiceDescription::SwtSequence& type){
            // Estimate max needed size. I'm not sure DIM can handle reallocations of this buffer, so we avoid that...
            service->buffer.resize(type.swtData.size()*20 + 512);
            getLogger() << "Starting SWT publisher '" << service->description.dnsName << "' with "
              << type.swtData.size() << " commands(s) at interval "
              << service->description.interval.count() << "ms" << endm;
          }
      );
    ...
    void serviceUpdate(Service& service)
    {
      ...
      Visitor::apply(service.description.type,
        ...
        [&](const ServiceDescription::SwtSequence& type){
          auto& bar2 = *(mBars.at(service.description.linkInfo.serial).at(2));
          auto swt = Swt(bar2, service.description.linkInfo.link);
          result = writeSwtSequence(type.swtData, swt, service.description.linkInfo);
        }
      );
    ...
    template <typename Iter>
    static std::vector<Swt::SwtData> linesToSwtVector(Iter begin, Iter end)
    {
        std::vector<Swt::SwtData> datas;
        for (Iter i = begin; i != end; ++i)
        {
            const std::string& line = *i;
            if (!isLineComment(line))
            {
                Swt::SwtData data;
                convertStringToSwtHex(line, data.data);
    ...
    /// Writes an SWT sequence and returns a string of the read results separated by newline
    static std::string writeSwtSequence(const std::vector<Swt::SwtData>& commandDataPairs, Swt& swt,
      LinkInfo linkInfo)
    {
      std::stringstream resultBuffer;
      for (const auto& commandData : commandDataPairs) {
        try {
          swt.write(commandData);
          if (commandData.data[0] & 0x80000000)
          {
	    getLogger() << (b::format("data=0x%s") % convertSwtHexToString(commandData.data)).str() << endm;
	    resultBuffer << "0" << "\n";
          }
	  else
	  {
            Swt::SwtData result = swt.read();
            getLogger() << (b::format("data=0x%s result=0x%s") % convertSwtHexToString(commandData.data) %
              convertSwtHexToString(result.data)).str() << endm;
            resultBuffer << convertSwtHexToString(result.data) << '\n';
	  }
        } catch (const SwtException &e) {
          // If an SCA error occurs, we stop executing the sequence of commands and return the results as far as we got
          // them, plus the error message.
          getLogger() << InfoLogger::InfoLogger::Error
            << (b::format("SWT_SEQUENCE data=0x%s serial=%d link=%d error='%s'") % convertSwtHexToString(commandData.data)
                % linkInfo.serial % linkInfo.link % e.what()).str() << endm;
          resultBuffer << e.what();
          break;
        }
      }
    ...
    /// RPC handler for SWT publish commands
    static std::string publishSwtSequenceStart(const std::string& parameter, std::shared_ptr<CommandQueue> queue,
      LinkInfo linkInfo)
    {
      getLogger() << "PUBLISH_SWT_SEQUENCE_START: '" << parameter << "'" << endm;
      ...
      // Convert command-data pair string sequence to binary format
      size_t skip = 2; // Number of arguments to skip for the array
      ServiceDescription::SwtSequence swt;
      swt.swtData = linesToSwtVector(params.begin() + skip, params.end());

      auto command = std::make_unique<CommandQueue::Command>();
      command->start = true;
      command->description.type = std::move(swt);
      command->description.dnsName = ServiceNames(linkInfo.serial, linkInfo.link).publishSwtSequenceSubdir(dnsName);
      command->description.interval = std::chrono::milliseconds(int64_t(b::lexical_cast<double>(interval) * 1000.0));
      command->description.linkInfo = linkInfo;
    ...
    /// RPC handler for SWT publish stop commands
    static std::string publishSwtSequenceStop(const std::string& parameter, std::shared_ptr<CommandQueue> queue,
      LinkInfo linkInfo)
    {
      getLogger() << "PUBLISH_SWT_SEQUENCE_STOP: '" << parameter << "'" << endm;

      auto command = std::make_unique<CommandQueue::Command>();
      command->start = false;
      command->description.type = ServiceDescription::Register();
      command->description.dnsName = ServiceNames(linkInfo.serial, linkInfo.link).publishSwtSequenceSubdir(parameter);
      command->description.interval = std::chrono::milliseconds(0);
      command->description.linkInfo = linkInfo;
    ...
    /// RPC handler for SWT read commands
    /*static std::string swtRead(const std::string&, BarSharedPtr bar2, LinkInfo linkInfo)
    {
        getLogger() << "SWT_READ" << endm;
        uint32_t data[3];
        uint32_t mon = Swt(*bar2, linkInfo.link).read(data);
        return (b::format("0x%x,0x%s") % mon % convertSwtHexToString(data)).str();
    }*/

    /// RPC handler for SWT write commands
    /*static std::string swtWrite(const std::string& parameter, BarSharedPtr bar2, LinkInfo linkInfo)
    {
        getLogger() << "SWT_WRITE: '" << parameter << "'" << endm;
        uint32_t data[3];
        convertStringToSwtHex(parameter, data);
        uint32_t mon = Swt(*bar2, linkInfo.link).write(data);
        return (b::format("0x%x") % mon).str();
    }*/
    ...
    static std::string scaBlobWrite(const std::string& parameter, BarSharedPtr bar2, LinkInfo linkInfo, bool sca_swt)
    {
      getLogger() << (sca_swt ? "SCA_SEQUENCE size=" : "SWT_SEQUENCE size=") << parameter.size() << " bytes" << endm;
      ...
      {
          auto commandDataPairs = linesToSwtVector(lines.begin(), lines.end());
          auto swt = Swt(*bar2, linkInfo.link);
          return writeSwtSequence(commandDataPairs, swt, linkInfo);
      }
    }
